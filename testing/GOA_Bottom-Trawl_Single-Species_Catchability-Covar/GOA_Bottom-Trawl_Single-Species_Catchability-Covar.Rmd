...
#Specifying **catchability** covariates for encounter probability **only**

Given you assumptions about the sampling process, it may make more sense to estimate the effect of the **catchability** covariates on the **encounter probability** component of the delta-model only. 
To do so, we must:

* Modify the `Tmb_List_...` object by...
  + Extracting the `$Map` object, as: `Map = TmbList$Map`
  + Modifying `Map` to turn off estimation of `lambda2_k`, as: `Map[["lambda2_k"]] = rep(NA, length(TmbList$Parameters$lambda2_k))`
* Recompile the `TmbList_...` with `VAST::BuildTMB_Fn()`

As an example we will update, recompile, and re-fit **Model 1** and **Model 2**, so that the effect of **tow duration** and **gear temperature** is only linked to encounter probability. We will refer to these as **Model 1b** and **Model 2b**.

##**Model 1b:** tow duration effect on encounter probability **only**
...
Next, lets turn off estimation of `lambda2_k`, lambda_k being the effect of **catchability** covariates, and **2** indicating this is for the *positive catch rate* component of the delta-model.

```{r}
Map_1b[["lambda2_k"]] = as.factor(rep(NA, length(TmbList_Mod1$Parameters$lambda2_k)))
```
...
#Recompile
ModList_Mod1b = VAST::Build_TMB_Fn(TmbData = TmbData_Mod1, RunDir = DateFile_Mod1b,
                                Version = Version, RhoConfig = RhoConfig, 
                                loc_x = Spatial_List$loc_x,
...
```{r, results="hide"}
Opt_Mod1b = TMBhelper::Optimize(obj = Obj_Mod1, lower = TmbList_Mod1b[["Lower"]],
