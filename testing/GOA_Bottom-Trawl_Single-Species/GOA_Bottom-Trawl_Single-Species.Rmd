...
grid_size_km = 25
n_x = 10#c(100, 250, 500, 1000, 2000)[1]
...
#Model with Vessel Effects

In some cases we might suspect there are **vessel effects** that influence overdispersion in our sampling process. Therefore it might make sense to directly estimate these effects.

To do so, we must first add our vessel indicator... 

##Add **vessel effects** to `Data_Geostat`

The vessel number is retained within `load.data` in the attribute `VESSEL.x`, so we will add that to our `Data_Geostat`.
This will be referenced and specified below when we call `Data_Fn()`, as argument `v_i`.

```{r add vessel effects}
Data_Geostat$Vessel = as.factor(load.data$VESSEL.x)
```

##Turn **ON** estimation of vessel effects

In order to turn on estimation of **vessel** effects, we need to update `OverdispersionConfig` vector, recalling that the **1st** element is for the encounter probability component of the delta model and the **2nd** element is for positive catch rate.

```{r update OverdispersionConfig}
OverdispersionConfig_2  = c(Delta1 = 1, Delta2 = 1)
```

##Reconstruct the model

Next, we will rebuilding the various components of our VAST model.

Create **new** `DateFile` so all of our model components are in a separate place, and our `.cpp` files don't get confused. 

*Note: This may not be entirely necessary, but is mostly Curry being festidious. 

```{r}
DateFile_2 <- paste0(DateFile,"/Vessel_Effect/")
dir.create(DateFile_2)
```

###Build individual model lists and components with the updated `OverdispersionConfig` and `Data_Geostat` list.

```{r, message=FALSE, tidy=TRUE}
if(length(species.codes) > 1 & combineSpecies==FALSE) {
  #MULTISPECIES
  TmbData_2 = VAST::Data_Fn(Version=Version, FieldConfig=FieldConfig,
                    OverdispersionConfig=OverdispersionConfig_2,
                    RhoConfig=RhoConfig, ObsModel=ObsModel, c_i=as.numeric(Data_Geostat[,'spp'])-1,
                    b_i=Data_Geostat[,'Catch_KG'], a_i=Data_Geostat[,'AreaSwept_km2'],
                    v_i=as.numeric(Data_Geostat[,'Vessel'])-1, s_i=Data_Geostat[,'knot_i']-1,
                    t_i=Data_Geostat[,'Year'], a_xl=Spatial_List$a_xl, MeshList=Spatial_List$MeshList,
  ...
  #SINGLE SPECIES
  TmbData_2 = VAST::Data_Fn(Version = Version, FieldConfig = FieldConfig,
                    OverdispersionConfig = OverdispersionConfig_2, RhoConfig = RhoConfig,
                    ObsModel = ObsModel, c_i = rep(0, nrow(Data_Geostat)),
                    b_i = Data_Geostat[, "Catch_KG"], a_i = Data_Geostat[,"AreaSwept_km2"],
                    v_i = as.numeric(Data_Geostat[,"Vessel"]) - 1,
                    s_i = Data_Geostat[, "knot_i"] - 1, t_i = Data_Geostat[, "Year"],
...
```{r, message=FALSE, results="hide", tidy=TRUE}
TmbList_2 = VAST::Build_TMB_Fn(TmbData = TmbData_2, RunDir = DateFile_2,
                                Version = Version, RhoConfig = RhoConfig, loc_x = Spatial_List$loc_x,
                                Method = Method)
Obj_2 = TmbList_2[["Obj"]]
```

###Do estimation for **new** model with vessel effects
Fit VAST model to the data by optimizing the TMB function.

```{r, results="hide"}
Opt_2 = TMBhelper::Optimize(obj = Obj_2, lower = TmbList_2[["Lower"]],
                          upper = TmbList_2[["Upper"]], getsd = TRUE, savedir = DateFile_2,
                          bias.correct = bias.correct)
```
...
```{r}
Report_2 = Obj_2$report()

Save_2 = list("Opt"=Opt_2, "Report"=Report_2, "ParHat"=Obj_2$env$parList(Opt_2$par), 
            "TmbData"=TmbData_2)

save(Save_2, file=paste0(DateFile_2,"Save.RData"))
```
...
Lets look at the vessel effects that were estimated by the model...

```{r}
pander::pandoc.table( Opt_2$diagnostics[,c('Param','Lower','MLE','Upper','final_gradient')] )
...
##Compare models with and without **Vessel Effects** with AIC

We can use AIC as a criteria for evaluating whether or not to include vessel effects. 
