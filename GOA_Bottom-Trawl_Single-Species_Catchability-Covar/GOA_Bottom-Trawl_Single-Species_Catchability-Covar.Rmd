...
Given you assumptions about the sampling process, it may make more sense to estimate the effect of the **catchability** covariates on the **positive catch rate** component of the delta-model only. 
To do so, we must:
...
As an example we will update, recompile, and re-fit **Model 1** and **Model 2**, so that the effect of **tow duration** and **gear temperature** is only linked to encounter probability.
...
##**Model 2b:** tow duration effect on encounter probability **only**
